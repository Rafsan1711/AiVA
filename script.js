...
/*
  script.js for AiVA Code Studio plugin + integration
  - Assumes Firebase SDK is loaded and firebase.initializeApp(...) has already been executed
  - Assumes `auth` and `database` variables (firebase.auth(), firebase.database()) are available globally
  - Safe: if Firebase not initialized we log and disable plugin features (we do not modify existing config)
  - Server base: will use window.SERVER_BASE if present, otherwise defaults to the original server URL.
*/

/* -----------------------
   Configuration & State
   ----------------------- */
const SERVER_BASE = window.SERVER_BASE || "https://aiva-gwm9.onrender.com";

let currentUserLocal = null;
let basePath = null; // users/{uid}/plugins/code_editor
let userFilesRef = null;
let userChatRef = null;

let filesCache = {};       // filename -> meta
let chatMessages = [];     // list of messages for plugin chat

// Elements (IDs created in index.html)
const el = {
  toggleCodePluginBtn: document.getElementById('toggleCodePluginBtn'),
  codeBadge: document.getElementById('codeBadge'),
  codePluginModal: document.getElementById('codePluginModal'),
  closeCodePlugin: document.getElementById('closeCodePlugin'),
  deleteCodePlugin: document.getElementById('deleteCodePlugin'),
  ...
  cancelCreateFileBtn: document.getElementById('cancelCreateFileBtn'),
  ...
  deleteCodeModal: document.getElementById('deleteCodeModal'),
  confirmDeleteCheckbox: document.getElementById('confirmDeleteCheckbox'),
  confirmDeleteCode: document.getElementById('confirmDeleteCode'),
  cancelDeleteCode: document.getElementById('cancelDeleteCode'),
  exportAllBtn: document.getElementById('exportAllBtn'),
  syncNowBtn: document.getElementById('syncNowBtn')
};

// guard: ensure required DOM elements exist
if (!el.toggleCodePluginBtn) {
  console.warn('Code Studio UI elements not found. Make sure index.html contains Code Studio markup.');
}

/* -----------------------
   Utilities
   ----------------------- */
function log(...args) { console.log('[CodeStudio]', ...args); }
function err(...args) { console.error('[CodeStudio]', ...args); }
...
// Parse AI replies for ===FILE: name === ```lang\ncode``` blocks and generic fences
function parseAICodeBlocks(text) {
  ...
  // fallback to fenced code blocks
  const fenceRegex = /```([a-zA-Z0-9+-]*)\n([\s\S]*?)```/g;
  while ((m = fenceRegex.exec(text)) !== null) {
    const lang = m[1] || 'text';
    const code = m[2].replace(/\r\n/g, '\n').trim();
    // try to determine filename from preceding lines (simple heuristic)
  ...
  // if nothing found, include raw text as single file
  if (results.length === 0) results.push({ filename: 'generated.txt', lang: 'text', code: text });
  return results;
}

// Extract JSON object substring from text (naive)
...
async function callServerQuery({ message, conversationType = 'code_plugin', max_tokens = 1200 }) {
  try {
    ...
    err('callServerQuery failed:', e);
    throw e;
  }
}

/* -----------------------
   Firebase listeners & operations
   ----------------------- */
...
function listenFilesRealtime() {
  ...
  filesCache = {};
  el.filesList.innerHTML = '<div class="muted">Loading files…</div>';
  userFilesRef.on('value', snap => {
    const all = snap.val() || {};
    filesCache = all;
    renderFilesList(all);
  }, e => {
    err('files listener error', e);
    el.filesList.innerHTML = '<div class="muted">Error loading files</div>';
  });
}
...
function renderFilesList(filesObj) {
  ...
  if (names.length === 0) {
    el.filesList.innerHTML = '<div class="muted">No files yet. Create one.</div>';
    return;
  }
  names.forEach(name => {
    ...
    left.className = 'meta';
    left.innerHTML = `<strong>${escapeHtml(name)}</strong><div class="muted" style="font-size:12px;">v${meta.version||1} • ${meta.modifiedBy ? 'by user' : ''}</div>`;
...
function renderChatArea() {
  ...
  for (const m of chatMessages || []) {
    ...
    // If AI message contains code fences, render parsed code cards below it
...
function showInlineEditor(filename, content) {
  ...
  el.currentFilename.textContent = filename;
  el.commitFileBtn.disabled = false;
  el.downloadFileBtn.disabled = false;
  el.generatedArea.innerHTML = '';
...
async function openFile(filename, edit = false) {
  try {
...
function closeOpenFile() {
  ...
  el.currentFilename.textContent = 'No file opened';
  el.commitFileBtn.disabled = true;
  el.downloadFileBtn.disabled = true;
  el.generatedArea.innerHTML = '';
}
...
async function generateFeatureSuggestion() {
  ...
  el.generateFeatureBtn.disabled = true;
  el.generateFeatureBtn.textContent = 'Generating...';
  try {
    ...
    el.featureInput.value = suggested.slice(0, 900);
  } catch (e) {
    err('generateFeatureSuggestion error', e);
    alert('Feature generation failed (server may not be configured)');
  } finally {
    el.generateFeatureBtn.disabled = false;
    el.generateFeatureBtn.textContent = 'Generate feature with AI';
  }
}

async function sendFeature() {
  const text = (el.featureInput.value || '').trim();
  if (!text) { alert('Please provide a feature description'); return; }
  // push user message
  await pushChatMessage({ role: 'user', content: text });
  renderChatArea();
  // ask AI for "what I understood" overview
  ...
  try {
    ...
    renderChatArea();
    // show satisfaction buttons after rendering (UI shows them permanently but we toggle)
    el.finalSatisfiedBtn.classList.remove('hidden');
    el.finalNotSatisfiedBtn.classList.remove('hidden');
  } catch (e) {
    err('sendFeature error', e);
...
function renderAIGeneratedCode(text) {
  ...
  parsed.forEach(item => {
    ...
    el.generatedArea.appendChild(card);
    if (window.Prism) Prism.highlightElement(code);
  });
  lastAIHtml = el.generatedArea.innerHTML;
  return lastAIHtml;
}

/* Debugging: ask server to return JSON { issues: [{line, message}], fixed_code } */
async function debugCode(filename, code) {
  ...
  el.generatedArea.prepend(debugCard);
  try {
...
If there are no issues, return issues: [] and fixed_code identical to input.`;
    ...
    err('debugCode error', e);
    debugCard.innerHTML = `<div class="muted">Debug failed: ${escapeHtml(String(e.message || e))}</div>`;
  }
}

/* Finalize: take last AI message that contains code and commit those files into files path; then clear chat */
async function finalizeAndCommit() {
  ...
  if (!chatMessages || chatMessages.length === 0) { alert('No AI content to commit'); return; }
...
async function deletePluginData() {
  ...
  try {
    ...
    el.deleteCodeModal.classList.add('hidden');
    el.codeBadge.textContent = 'Disabled';
    el.toggleCodePluginBtn.textContent = 'Enable Plugin';
    alert('Plugin data deleted.');
  } catch (e) {
    err('deletePluginData', e);
    alert('Delete failed');
  }
}
...
/* -----------------------
   ...
   ----------------------- */
function wireUI() {
  // Toggle plugin enable/open
  el.toggleCodePluginBtn?.addEventListener('click', () => {
    try {
      const saved = JSON.parse(localStorage.getItem('aivaEnabledPlugins') || '{}');
      const now = !saved['code_editor'];
      saved['code_editor'] = now;
      localStorage.setItem('aivaEnabledPlugins', JSON.stringify(saved));
      el.codeBadge.textContent = now ? 'Enabled' : 'Disabled';
      el.toggleCodePluginBtn.textContent = now ? 'Open Plugin' : 'Enable Plugin';
  ...
  // open/close modal
  el.closeCodePlugin?.addEventListener('click', closePluginModal);
  // create form toggle
  el.createFileToggleBtn?.addEventListener('click', () => {
    el.createFileForm.classList.toggle('hidden');
    if (!el.createFileForm.classList.contains('hidden')) el.newFileName.focus();
    else { el.newFileName.value = ''; el.newFileContent.value = ''; }
  });
  el.cancelCreateFileBtn?.addEventListener('click', () => { el.createFileForm.classList.add('hidden'); el.newFileName.value=''; el.newFileContent.value=''; });
  el.createFileBtn?.addEventListener('click', async () => {
    const fname = (el.newFileName.value || '').trim();
    const content = el.newFileContent.value || '';
    if (!fname) { alert('Enter filename'); return; }
    await createFile(fname, content);
    el.newFileName.value = ''; el.newFileContent.value = ''; el.createFileForm.classList.add('hidden');
  });
  el.generateFeatureBtn?.addEventListener('click', generateFeatureSuggestion);
  el.sendFeatureBtn?.addEventListener('click', sendFeature);
  el.finalSatisfiedBtn?.addEventListener('click', finalizeAndCommit);
  el.finalNotSatisfiedBtn?.addEventListener('click', () => { alert('Please update the feature input and Send again.'); });

  el.commitFileBtn?.addEventListener('click', async () => {
  ...
  el.downloadFileBtn?.addEventListener('click', async () => {
  ...
  // delete plugin modal
  el.deleteCodePlugin?.addEventListener('click', () => {
    el.deleteCodeModal.classList.remove('hidden');
    el.confirmDeleteCheckbox.checked = false;
    el.confirmDeleteCode.disabled = true;
  });
  el.confirmDeleteCheckbox?.addEventListener('change', () => {
    el.confirmDeleteCode.disabled = !el.confirmDeleteCheckbox.checked;
  });
  el.cancelDeleteCode?.addEventListener('click', () => el.deleteCodeModal.classList.add('hidden'));
  el.confirmDeleteCode?.addEventListener('click', deletePluginData);

  // placeholder actions
  el.exportAllBtn?.addEventListener('click', () => alert('Export not implemented in this build.'));
  el.syncNowBtn?.addEventListener('click', () => alert('Realtime sync is active; explicit sync not required.'));
}

/* -----------------------
   Plugin modal open/close
   ----------------------- */
function openPluginModal() {
  if (!window.firebase || !firebase.auth) { alert('Firebase not initialized. Plugin requires auth.'); return; }
  ...
  listenChatRealtime();
  // show modal
  el.codePluginModal.classList.add('open');
}

function closePluginModal() {
  el.codePluginModal.classList.remove('open');
...
if (window.auth && typeof auth.onAuthStateChanged === 'function') {
  auth.onAuthStateChanged(user => {
    ...
    // update button text & badge using localStorage
    const saved = JSON.parse(localStorage.getItem('aivaEnabledPlugins') || '{}');
    const enabled = !!saved['code_editor'];
    if (el.codeBadge) el.codeBadge.textContent = enabled ? 'Enabled' : 'Disabled';
    if (el.toggleCodePluginBtn) el.toggleCodePluginBtn.textContent = enabled ? 'Open Plugin' : 'Enable Plugin';
    if (user && enabled) {
      // ensure refs set (but don't open modal automatically)
      setupPluginRefs(user);
    }
  });
} else {
  log('Firebase auth not found. Plugin will not initialize until firebase is available.');
}
...
function init() {
  ...
  try {
    ...
    if (saved['code_editor']) {
      if (el.codeBadge) el.codeBadge.textContent = 'Enabled';
      if (el.toggleCodePluginBtn) el.toggleCodePluginBtn.textContent = 'Open Plugin';
    } else {
      if (el.codeBadge) el.codeBadge.textContent = 'Disabled';
      if (el.toggleCodePluginBtn) el.toggleCodePluginBtn.textContent = 'Enable Plugin';
    }
  } catch (e) { /* ignore */ }
}
init();
